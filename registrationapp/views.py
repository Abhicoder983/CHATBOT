...
from registrationapp.models import registrationModel , messageModel
...
def loggedIn(request):
    ...
    # Fetch user messages
    data = messageModel.objects.filter(user=email).values('ai_message', 'user_message')
    print(data)
    # Fetch user name correctly
    user = registrationModel.objects.filter(email=email)
    name = user[0].name if user else 'Guest'
...
def get_ai_info(request):
    auth_login=request.session.get('auth_email2',None)
    allowed_referrer = "http://127.0.0.1:8000/loggedIn/"  
